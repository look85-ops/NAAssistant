...
def main():
    ...
    parser.add_argument("--prompt-only", action="store_true", help="Только вывести промпт, без генерации")
    args = parser.parse_args()

    # 1. Read instruction
    instruction_path = os.path.join(BASE_DIR, "docs", "course", "INSTRUCTION_ProfPlus_Path_v2.3.md")
    instruction = ""
    ...
    # 4. Output
    output_path = args.output or os.path.join(BASE_DIR, "docs", "course", f"M{args.module}_D{args.day}_prompt.md")
    ...
    if args.prompt_only:
        ...
        print(f"Размер промпта: {len(prompt)} chars (~{len(prompt)//4} токенов)")
    else:
        # Just save the prompt for manual use with LLM
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(prompt)
        print(f"Промпт сохранён: {output_path}")
        print(f"\nСкопируй этот промпт в Claude/ChatGPT для генерации сценария.")
        print(f"Или запусти с --llm для авто-генерации (требуется API-ключ).")
